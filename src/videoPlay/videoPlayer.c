...
static status_e SDL2_ReSource_init(const SDL_Rect * pSDLRect, SDL_Window * pSDLWindow, SDL_Renderer * pSDLRender, SDL_Texture * pSDLTexture)
{
    ...
    pSDLWindow = SDL_CreateWindow("this is sample video player", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 
    ...
    pSDLTexture = SDL_CreateTexture(pSDLRender, SDL_PIXELFORMAT_IYUV, SDL_TEXTUREACCESS_STREAMING, pSDLRect->w, pSDLRect->h);
    if(!pSDLTexture)
    {
        printf("[%s][%d]! SDL_CreateRenderer failed\n", __func__, __LINE__);
        return STATUS_FALSE;
